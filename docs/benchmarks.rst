...
The deterministic benchmark used small, 100-record, and 1,000-record nested payloads generated by ``benchmark_security_hardening.py``. Record fields depend only on their zero-based index, so no pseudorandom seed or hidden input is required.
