...
[[json2xml/cli.py#create_parser]] defines the user-facing flags. `CLIApplication` owns source resolution and output, while [[json2xml/cli.py#read_input]] and [[json2xml/cli.py#main]] remain stable wrappers. [[json2xml/cli.py#CLIApplication#_resolve_input_format]] is the single place a source's framing is decided: `--jsonl` and `--no-jsonl` win outright, otherwise a `.jsonl` or `.ndjson` suffix selects line framing. JSON Lines then takes the streaming path through [[json2xml/jsonl.py#stream_jsonl_to_xml]] or, when the requested layout needs the whole document, the materialized path through [[json2xml/utils.py#readfromjsonl]]. Both meet the existing backend at [[json2xml/json2xml.py#Json2xml]].
