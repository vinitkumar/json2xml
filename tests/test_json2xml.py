...
class TestJson2xml:
    ...
    def test_native_budget_check_replaces_python_walk(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A native walk that completes within budget makes the Python walk redundant."""
        python_walk = Mock()
        monkeypatch.setattr(json2xml, "_validate_conversion_budget", python_walk)
        monkeypatch.setattr(
            json2xml.dicttoxml, "check_conversion_budget", Mock(return_value=True)
        )

        assert json2xml.Json2xml({"a": 1}).to_xml() is not None
        python_walk.assert_not_called()

    def test_python_walk_runs_when_native_check_declines(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        python_walk = Mock()
        monkeypatch.setattr(json2xml, "_validate_conversion_budget", python_walk)
        monkeypatch.setattr(
            json2xml.dicttoxml, "check_conversion_budget", Mock(return_value=False)
        )

        json2xml.Json2xml({"a": 1}, max_depth=7, max_items=9).to_xml()

        python_walk.assert_called_once_with({"a": 1}, 7, 9)

    def test_native_budget_violation_becomes_invalid_data_error(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            json2xml.dicttoxml,
            "check_conversion_budget",
            Mock(side_effect=ValueError("JSON item limit exceeded")),
        )

        with pytest.raises(InvalidDataError, match="JSON item limit exceeded"):
            json2xml.Json2xml({"a": 1}).to_xml()
