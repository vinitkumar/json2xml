...
class TestJson2xml:
    ...
    def test_json_to_xml_defaults_to_compact_output(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_conversion_resource_limits(self, data: Any, limits: _ConversionLimits) -> None:
    ...
    def test_output_limit_accepts_payload_when_generated_xml_fits(self) -> None:
        ...
        xml = json2xml.Json2xml({"nums": list(range(50))}, max_output_bytes=5_000).to_xml()

        assert isinstance(xml, bytes)
        assert len(xml) < 5_000

    @pytest.mark.parametrize("value", [0, -1, True, 1.5, "10"])
    def test_conversion_resource_limits_require_positive_integers(self, value: Any) -> None:
    ...
    def test_pretty_print_does_not_reparse_a_dom(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_pretty_print_keeps_character_data_on_one_line(self) -> None:
        ...
        result = json2xml.Json2xml({"a": {"@attrs": {"x": "1"}, "@val": "text"}}, pretty=True).to_xml()
    ...
    def test_pretty_print_indents_nested_containers_during_serialization(self) -> None:
        ...
        result = json2xml.Json2xml({"outer": {"leaf": "value", "items": [1]}}, pretty=True).to_xml()
